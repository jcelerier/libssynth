...
public:
  ...
  Matrix4<scalar> operator*(const Matrix4<scalar>& rhs) const
  {
    ...
    for (int x = 0; x < 4; x++)
    {
      ...
      {
        for (int i = 0; i < 4; i++)
          m.getRef(x, y) += (this->at(x, i) * rhs.at(i, y));
      }
    }
    return m;
  };
